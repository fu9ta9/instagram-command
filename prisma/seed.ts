...
async function main() {
  // テストユーザーの作成（upsertを使用）
  const user = await prisma.user.upsert({
    where: { email: 'sakainoblig@gmail.com' },
    update: {
      name: 'Test User',
  ...
  // テストIGAccountの作成（upsertを使用）
  const igAccount = await prisma.iGAccount.upsert({
    where: { 
  ...
  console.log(`IGアカウント: ${igAccount.username}`)
}
