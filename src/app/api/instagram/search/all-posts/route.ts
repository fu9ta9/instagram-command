...
export async function GET(request: Request) {
  try {
    ...
    // すべての投稿を取得する関数
    async function fetchAllPosts(username: string) {
      let allPosts: any[] = [];
      let hasMore = true;
      let afterToken = null;
      let attempts = 0;
      ...
      while (hasMore && attempts < MAX_ATTEMPTS) {
        ...
        // APIエンドポイントを構築
        let apiUrl = `https://graph.facebook.com/v22.0/${igUserId}?fields=business_discovery.username(${username}){username,name,profile_picture_url,followers_count,media_count,media${afterToken ? `.after(${afterToken})` : ''}{id,comments_count,like_count,media_url,permalink,timestamp,media_type,thumbnail_url}}&access_token=${accessToken}`;
    ...
    // すべての投稿を取得
    const result = await fetchAllPosts(accountId);
